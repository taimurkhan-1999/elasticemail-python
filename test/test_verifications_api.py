# coding: utf-8

"""
    Elastic Email REST API

    This API is based on the REST API architecture, allowing the user to easily manage their data with this resource-based approach.    Every API call is established on which specific request type (GET, POST, PUT, DELETE) will be used.    The API has a limit of 20 concurrent connections and a hard timeout of 600 seconds per request.    To start using this API, you will need your Access Token (available <a target='_blank' href='https://app.elasticemail.com/marketing/settings/new/manage-api'>here</a>). Remember to keep it safe. Required access levels are listed in the given request’s description.    Downloadable library clients can be found in our Github repository <a target='_blank' href='https://github.com/ElasticEmail?tab=repositories&q=%22rest+api%22+in%3Areadme'>here</a>

    The version of the OpenAPI document: 4.0.0
    Contact: support@elasticemail.com
    Generated by OpenAPI Generator (https://openapi-generator.tech)

    Do not edit the class manually.
"""  # noqa: E501


import unittest

from ElasticEmail.api.verifications_api import VerificationsApi


class TestVerificationsApi(unittest.TestCase):
    """VerificationsApi unit test stubs"""

    def setUp(self) -> None:
        self.api = VerificationsApi()

    def tearDown(self) -> None:
        pass

    def test_verifications_by_email_delete(self) -> None:
        """Test case for verifications_by_email_delete

        Delete Email Verification Result
        """
        pass

    def test_verifications_by_email_get(self) -> None:
        """Test case for verifications_by_email_get

        Get Email Verification Result
        """
        pass

    def test_verifications_by_email_post(self) -> None:
        """Test case for verifications_by_email_post

        Verify Email
        """
        pass

    def test_verifications_files_by_id_delete(self) -> None:
        """Test case for verifications_files_by_id_delete

        Delete File Verification Result
        """
        pass

    def test_verifications_files_by_id_result_download_get(self) -> None:
        """Test case for verifications_files_by_id_result_download_get

        Download File Verification Result
        """
        pass

    def test_verifications_files_by_id_result_get(self) -> None:
        """Test case for verifications_files_by_id_result_get

        Get Detailed File Verification Result
        """
        pass

    def test_verifications_files_by_id_verification_post(self) -> None:
        """Test case for verifications_files_by_id_verification_post

        Start verification
        """
        pass

    def test_verifications_files_post(self) -> None:
        """Test case for verifications_files_post

        Upload File with Emails
        """
        pass

    def test_verifications_files_result_get(self) -> None:
        """Test case for verifications_files_result_get

        Get Files Verification Results
        """
        pass

    def test_verifications_get(self) -> None:
        """Test case for verifications_get

        Get Emails Verification Results
        """
        pass


if __name__ == '__main__':
    unittest.main()
